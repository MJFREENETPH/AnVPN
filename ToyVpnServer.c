...
static void *recv_write(void *ptr) {
    ...
    char packet[32767];
    int length, decodedPacketLength;
    decodedPacketLength=32767;
    unsigned char *decodedPacket = (unsigned char*) malloc((sizeof(char)* decodedPacketLength));
    ...
    do {
        ...
        decodedPacketLength = b64_decode(packetPointer,length,decodedPacket);
        //printf("received %d bytes from socket\n", length);
        if (decodedPacket[0] != 0) {
            if ((length = write(interface, decodedPacket, decodedPacketLength)) == -1) {
                perror("write");
                break;
            }
